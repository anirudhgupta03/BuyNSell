...
<br>
    ...
    <?php
        ...
        if(isset($_SESSION['user'])) {
            ...
            $query = "Select * FROM products WHERE status = 'On Sale' AND uid <> $usr";
        }
        else{
            // $usr = json_decode( json_encode($row_c -> uid), true);
            $query = "Select * FROM products WHERE status = 'On Sale'";
        }
        ...
        <?php
            ...
            foreach ($ans as $value) {
                $query1 = "select * from products where pro_id = '$value'";
                    ...
                    ?>
                        ...
                        <h2 class="card-title" style="float:right;"><a href="view_product.php?pro_id=<?php echo $row_q1 -> pro_id; ?>" class="btn btn-warning">Buy</a></h2>
                                ...
                                <div class="card-body">
                                    ...
                                    if(isset($_SESSION['user'])) {
                                        $usr = json_decode( json_encode($row_c -> uid), true);
                                        $query_for_search = "insert into product_search (pro_id, uid) values ($row_q1->pro_id, $usr)";
                                        $run_query_for_search = $con->query($query_for_search);
                                    }
